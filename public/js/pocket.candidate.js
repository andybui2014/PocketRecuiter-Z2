...
pocketCandidate.prototype = {
	...
	//$('#add-anotherjob').html('<strong>Save</strong>');
	 var status=$('#add-anotherjob').attr("data-status");
	 if(status=="view")
	 {
		$('#employment-form #add-anotherjob').attr('data-status','add');
		$('#Job_Functions').focus();
	 }
	$('#cancel').show();
	//$this.button('loading');
	...
	//alert(status);
					...
					if(status=='add'){
						$.post('./do-add-job-function',{data: $('#employment-form').serializeArray()},function(xhr){
							...
							if(xhr.info=='Total Percentage greater 100%' || xhr.info=='Percentage not empty' || xhr.info=='Percentage must be integer')
							{
								alert(xhr.info);
